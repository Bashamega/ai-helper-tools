            ...
            <section className=" bg-slate-400/30 m-10 p-5 rounded-lg h-full-container overflow-auto relative">
                {response ? (
                    ...
                    <div className=" w-full h-content flex  items-center justify-center max-h-[50%] lg:max-h-[80%]  h-full">
                        <div>
                            <Image src={"/gimini.png"} width={400} height={0} className="lg:block hidden"></Image>
                            <p className="text-2xl text-center font-bold">Please choose a topic</p>
                        </div>
                    </div>
                ) : (
                ...
                <div className="lg:absolute bottom-[2%] w-[90%] left-0 mx-[5%] bg-slate-400 p-5 rounded-lg text-lg  lg:flex lg:justify-evenly lg:h-[100px] lg:items-center">
                    <select className="bg-white cursor-pointer text-black rounded-md h-full p-2 w-full lg:w-1/4" value={prompt.type} onChange={(e) => { setPrompt({ ...prompt, type: e.target.value }) }}>
                    ...
                    <br></br>
                    <input disabled={prompt.type == 1} className="bg-white my-5 lg:my-0 text-black rounded-md h-full p-2 w-full lg:w-2/4 disabled:bg-slate-200" value={prompt.topic} placeholder="Type the topic" onChange={(e) => { setPrompt({ ...prompt, topic: e.target.value }) }} />
                    <button className="lg:w-1/5 w-full btn-grad rounded-lg disabled:cursor-auto disabled:shadow-none h-full " onClick={generate} disabled={prompt.topic == ""}>Generate</button>
                </div>
            </section>
        </main>
    )
